...
export default function AssignmentView({ assignment, onBack }) {
  ...
  useEffect(() => {
    if (assignment?.serialNumber) {
      api.getAssignmentProjects(assignment.serialNumber).then(projects => {
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', flexWrap: 'wrap', position: 'relative' }}>
        ...
        {assignment.serialNumber && (
          <button className="btn btn-secondary" onClick={() => {
            window.open(`https://tsdr.uspto.gov/#caseNumber=${assignment.serialNumber}&caseSearchType=US_APPLICATION&caseType=DEFAULT&searchType=statusSearch`, '_blank');
          }}>
            View on TSDR
          </button>
        )}
        {assignment.serialNumber && (
          <button className="btn btn-secondary" onClick={() => {
            window.open(`https://tmsearch.uspto.gov/search/search-detail/${assignment.serialNumber}`, '_blank');
          }}>
            View on TMSearch
          </button>
        )}
